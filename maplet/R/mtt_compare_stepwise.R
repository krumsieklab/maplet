    ...
    if(length(part) > 2){
      if(as.character(part[[2]]) == "D"){
        if(is.call(part[[3]]) && startsWith(as.character(part[[3]][[1]]), "mt_")){
          ...
          maplet:::mti_logstatus(glue::glue("   Running step {step} of test script."))
          D_step <- do.call(mt_fun, args = arg_lst)
          
          # load reference object
          maplet:::mti_logstatus(glue::glue("   Loading step {step} reference object."))
          ...
          maplet:::mti_logstatus(glue::glue("   Comparing step {step} of test script to reference object."))
