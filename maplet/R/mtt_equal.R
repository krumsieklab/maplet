...
mtt_equal <- function(D1, D2){
  ...
  if(length(D1) != 0){
    ...
    maplet:::mti_logstatus("   Comparing the assay, rowData, and colData data frames.")
  ...
  if(length(d1_stat_tables) == length(d2_stat_tables)){
    ...
    if(length(d1_stat_tables)!=0){
      maplet:::mti_logstatus("   Comparing the statistical results tables.")
  ...
  if(length(d1_pw_anno_dfs) == length(d2_pw_anno_dfs)){
    ...
    if(length(d1_pw_anno_dfs)!=0){
      maplet:::mti_logstatus("   Comparing the pathway annotation data frames.")
  ...
  if(!is.null(metadata(D1)$assays) && !is.null(metadata(D2)$assays)){
    maplet:::mti_logstatus("   Comparing the assay version lists.")
    # DO SOMETHING!
